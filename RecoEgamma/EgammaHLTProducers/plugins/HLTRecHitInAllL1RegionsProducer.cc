...
// this is a re-write of HLTRechitInRegionsProducer to be able to handle
// arbitary L1 collections as inputs in the process, some of the cruft was
// cleaned up but it mantains almost all the old behaviour think the only
// difference now is that it wont throw if its not ECALBarrel, ECALEndcap or
// ECAL PS rec-hit type
...
class L1RegionData : public L1RegionDataBase {
 ...
 public:
  L1RegionData(const edm::ParameterSet& para,
      ...
      : minEt_(para.getParameter<double>("minEt")),
        ...
        token_(consumesColl.consumes<T1>(
            para.getParameter<edm::InputTag>("inputColl"))) {}

  void getEtaPhiRegions(const edm::Event&,
                        std::vector<RectangularEtaPhiRegion>&,
                        const L1CaloGeometry&) const override;
  ...
  template <typename T2>
  static typename BXVector<T2>::const_iterator beginIt(
      const BXVector<T2>& coll) {
    return coll.begin(0);
  }
  template <typename T2>
...
class HLTRecHitInAllL1RegionsProducer : public edm::stream::EDProducer<> {
 ...
 private:
  L1RegionDataBase* createL1RegionData(
      const std::string&, const edm::ParameterSet&,
...
HLTRecHitInAllL1RegionsProducer<RecHitType>::HLTRecHitInAllL1RegionsProducer(
  ...
  for (auto& pset : l1InputRegions) {
    ...
    l1RegionData_.emplace_back(createL1RegionData(
        type, pset,
        consumesCollector()));  // meh I was going to use a factory but it was
                                // going to be overly complex for my needs
  }
  ...
  for (unsigned int collNr = 0; collNr < recHitLabels_.size(); collNr++) {
    recHitTokens_.push_back(
        consumes<RecHitCollectionType>(recHitLabels_[collNr]));
    produces<RecHitCollectionType>(productLabels_[collNr]);
  }
}
template <typename RecHitType>
void HLTRecHitInAllL1RegionsProducer<RecHitType>::fillDescriptions(
  ...
  recHitLabels.push_back(
      edm::InputTag("hltEcalRegionalEgammaRecHit:EcalRecHitsEB"));
  recHitLabels.push_back(
      edm::InputTag("hltEcalRegionalEgammaRecHit:EcalRecHitsEE"));
  recHitLabels.push_back(
      edm::InputTag("hltESRegionalEgammaRecHit:EcalRecHitsES"));
  ...
  emIsoPSet.addParameter<edm::InputTag>(
      "inputColl", edm::InputTag("hltL1extraParticles:NonIsolated"));
  ...
  emNonIsoPSet.addParameter<edm::InputTag>(
      "inputColl", edm::InputTag("hltL1extraParticles:Isolated"));
  l1InputRegions.push_back(emNonIsoPSet);

  // Why no Central Jets here? They are present in the python config, e.g.
  // OnLine_HLT_GRun.py SHarper: because these are the default parameters
  // designed to reproduce the original (no jets) behaviour
  //
  ...
  egPSet.addParameter<edm::InputTag>("inputColl",
                                     edm::InputTag("hltCaloStage2Digis"));
  ...
  jetPSet.addParameter<edm::InputTag>("inputColl",
                                      edm::InputTag("hltCaloStage2Digis"));
  ...
  descriptions.add(
      defaultModuleLabel<HLTRecHitInAllL1RegionsProducer<RecHitType>>(), desc);
}

template <typename RecHitType>
void HLTRecHitInAllL1RegionsProducer<RecHitType>::produce(
  ...
  std::for_each(l1RegionData_.begin(), l1RegionData_.end(),
                [&event, &regions,
                 l1CaloGeom](const std::unique_ptr<L1RegionDataBase>& input) {
                  input->getEtaPhiRegions(event, regions, *l1CaloGeom);
                });

  for (size_t recHitCollNr = 0; recHitCollNr < recHitTokens_.size();
       recHitCollNr++) {
    edm::Handle<RecHitCollectionType> recHits;
    event.getByToken(recHitTokens_[recHitCollNr], recHits);

    if (!(recHits.isValid())) {
      edm::LogError("ProductNotFound")
          << "could not get a handle on the "
          << typeid(RecHitCollectionType).name() << " named "
          << recHitLabels_[recHitCollNr].encode() << std::endl;
      continue;
    }
    ...
    if (!recHits->empty()) {
      const CaloSubdetectorGeometry* subDetGeom =
          caloGeomHandle->getSubdetectorGeometry(recHits->front().id());
      if (!regions.empty()) {
    ...
    //   std::cout <<"putting fileter coll in
    //   "<<filteredRecHits->size()<<std::endl;
...
template <typename RecHitType>
L1RegionDataBase*
HLTRecHitInAllL1RegionsProducer<RecHitType>::createL1RegionData(
    const std::string& type, const edm::ParameterSet& para,
    edm::ConsumesCollector&& consumesColl) {
  if (type == "L1EmParticle") {
    return new L1RegionData<l1extra::L1EmParticleCollection>(para,
                                                             consumesColl);
  } else if (type == "L1JetParticle") {
    return new L1RegionData<l1extra::L1JetParticleCollection>(para,
                                                              consumesColl);
  } else if (type == "L1MuonParticle") {
    return new L1RegionData<l1extra::L1MuonParticleCollection>(para,
                                                               consumesColl);
  } else if (type == "EGamma") {
    ...
    // this is a major issue and could lead to rather subtle efficiency losses,
    // so if its incorrectly configured, we're aborting the job!
    throw cms::Exception("InvalidConfig")
        << " type " << type
        << " is not recognised, this means the rec-hit you think you are "
           "keeping may not be and you should fix this "
...
void L1RegionData<L1CollType>::getEtaPhiRegions(
    const edm::Event& event, std::vector<RectangularEtaPhiRegion>& regions,
    const L1CaloGeometry&) const {
  edm::Handle<L1CollType> l1Cands;
  event.getByToken(token_, l1Cands);

  for (auto l1CandIt = beginIt(*l1Cands); l1CandIt != endIt(*l1Cands);
       ++l1CandIt) {
    if (l1CandIt->et() >= minEt_ && l1CandIt->et() < maxEt_) {
      ...
      regions.push_back(
          RectangularEtaPhiRegion(etaLow, etaHigh, phiLow, phiHigh));
    }
  }
}
...
typedef HLTRecHitInAllL1RegionsProducer<EcalRecHit>
    HLTEcalRecHitInAllL1RegionsProducer;
DEFINE_FWK_MODULE(HLTEcalRecHitInAllL1RegionsProducer);
typedef HLTRecHitInAllL1RegionsProducer<EcalUncalibratedRecHit>
    HLTEcalUncalibratedRecHitInAllL1RegionsProducer;
DEFINE_FWK_MODULE(HLTEcalUncalibratedRecHitInAllL1RegionsProducer);
typedef HLTRecHitInAllL1RegionsProducer<HGCRecHit>
    HLTHGCalRecHitInAllL1RegionsProducer;
DEFINE_FWK_MODULE(HLTHGCalRecHitInAllL1RegionsProducer);
typedef HLTRecHitInAllL1RegionsProducer<HGCUncalibratedRecHit>
    HLTHGCalUncalibratedRecHitInAllL1RegionsProducer;
DEFINE_FWK_MODULE(HLTHGCalUncalibratedRecHitInAllL1RegionsProducer);
