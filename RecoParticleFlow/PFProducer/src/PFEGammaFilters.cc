...
bool PFEGammaFilters::passElectronSelection(const reco::GsfElectron& electron,
  ...
  //TEMPORARY solution. 
  //Do not allow new EtaExtendedEle to enter PF, until ID, regression are in place.
  if  (!allowEEEinPF_) {
...
bool PFEGammaFilters::isElectronSafeForJetMET(const reco::GsfElectron& electron,
  ...
  //TEMPORARY solution. 
  //Do not allow new EtaExtendedEle to be SafeForJetMET, until ID, regression are in place.  
  if  (!allowEEEinPF_) {
    int nHitGsf= electron.gsfTrack()->numberOfValidHits();
    double absEleEta = fabs(electron.eta());
    if ( (absEleEta>2.5) && (nHitGsf<5) ) {
