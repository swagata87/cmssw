...
public:
  ...
  std::vector<float> localCovariancesNC(const reco::BasicCluster &cluster, const edm::EventSetup* eventSetup_ , float w0 = 4.7, float mult=1.25) {
    return ClusterTools::localCovariancesNC(cluster, getEcalRecHitCollection(cluster), topology_, eventSetup_, w0, mult);
  }
