...
public:
  ...
  static std::vector<float> localCovariances_NoiseCleaned(const reco::BasicCluster &cluster, 
							  const EcalRecHitCollection* recHits, 
							  const CaloTopology *topology, 
							  const CaloGeometry* geometry, 
...
template<bool noZS>
std::vector<float> EcalClusterToolsT<noZS>::localCovariances_NoiseCleaned(const reco::BasicCluster &cluster, const EcalRecHitCollection* recHits,const CaloTopology *topology, const CaloGeometry* geometry, const edm::EventSetup* eventSetup_, float w0, float mult)
{
  float e_5x5 = e5x5( cluster, recHits, topology );
  float covEtaEta, covEtaPhi, covPhiPhi;
